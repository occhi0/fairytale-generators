...
import { Camera, Sparkles, Copy, RefreshCw, BookOpen, Palette, Eye } from 'lucide-react';
...
Rispondi SOLO con un oggetto JSON:
{
  ...
  "suggerimentiUso": "consigli per usare il character sheet nelle illustrazioni successive"
      ...
      // const risposta = await window.claude.complete(prompt);
       const dati = {
        analisiPersonaggio: "Luca è un piccolo coniglietto dal pelo bianco come la neve, con orecchie lunghe e soffici. È un personaggio gentile, coraggioso e altruista, che vive in un prato verde e fiorito.",
        promptCharacterSheet: "Character sheet of a small, cute baby rabbit named Luca, white fluffy fur, long soft ears, wearing a tiny blue vest. Multiple poses for character consistency: front view, side profile, 3/4 view, and a happy expression. Children's book illustration style, soft watercolor technique, gentle pastel color palette (light greens, blues, yellows), on a simple white background. --ar 16:9",
        suggerimentiUso: "Usa questa immagine come riferimento per tutte le illustrazioni successive per mantenere l'aspetto di Luca coerente in tutta la storia."
      };

      setAnalisiPersonaggio(dati.analisiPersonaggio);
      setPromptGenerato(dati.promptCharacterSheet);

    } catch (errore) {
      console.error('Errore:', errore);
      alert('Errore nella generazione. Riprova per favore.');
    } finally {
  ...
  const copiaPrompt = () => {
    if (promptGenerato) {
      navigator.clipboard.writeText(promptGenerato);
